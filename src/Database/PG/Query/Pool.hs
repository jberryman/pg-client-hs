...
initPGPool ci cp logger =
  ...
  where
    ...
    creator   = do
      ...
      ctr     <- newIORef 0
      table   <- HI.new
      return $ PGConn pqConn (cpAllowPrepare cp) retryP logger ctr table createdAt
...
-- expiring the connection after a time so that memory at least can't
-- accumulate unbounded in long-lived connections.
--
...
withExpiringPGconn :: (MonadBaseControl IO m, MonadIO m)=> PGPool -> (PGConn -> m a) -> m a
withExpiringPGconn pool f = do
  ...
  handleLifted (\PGConnectionStale -> withExpiringPGconn pool f) $ do
    RP.withResource pool $ \connRsrc@PGConn{pgCreatedAt} -> do
      now <- liftIO $ getCurrentTime
      when (now `diffUTCTime` pgCreatedAt > lifetime) $ do
...
-- internal, see withExpiringPGconn
