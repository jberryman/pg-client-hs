...
retryOnConnErr pgConn action =
  ...
  where
    ...
    PGConn _ _ retryP logger _ _ _ = pgConn
...
resetPGConn (PGConn conn _ _ _ ctr ht _) = do
...
prepare
  ...
  -> [PQ.Oid]
  -> PGExec RemoteKey
prepare (PGConn conn _ _ _ counter table _) t tl = do
...
execQuery pgConn pgQuery = do
  ...
  where
    PGConn conn allowPrepare _ _ _ _ _ = pgConn
